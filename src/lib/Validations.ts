...
class Validations {
	public static isValidPlace(row: number, column: number): boolean {
		return (
			column < BoardConstants.COLUMNS &&
			row < BoardConstants.ROWS &&
			column >= 0 &&
			row >= 0
		);
	}
	...
	public static isValidPosition(userInput: string, board: Board): boolean {
		...
		return (
			hasSameColor && this.isValidPlace(position.getRow(), position.getColumn())
		);
	}

	public static notStepBack(
		figurColor: Color,
		row: number,
